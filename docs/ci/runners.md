...
Android Play and ThirdParty unit/lint pairs keep separate Gradle processes to avoid overlapping expensive test and lint work. Both commands capture one UTC build timestamp through the existing `openclawBuildTimestamp` property, avoiding `BuildConfig` regeneration solely because lint starts later. Phone test classes on Blacksmith run in at most two isolated JVMs, bounded by available processors, with the existing 1 GiB heap per JVM. Classes remain sequential within each JVM; hosted tests keep one JVM. Unit-only compatibility tasks and other Android rows retain their existing metadata behavior; task coverage, runner labels, job concurrency, and deadlines are unchanged.
